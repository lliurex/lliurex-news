        ...
        return fetch(url, {headers})
            ...
            .catch(() => {
                ...
                if (!options.ignoreErrors && !this.error) {
                    this.set('error', this.intl.t('Uh-oh! Trouble reaching the Unsplash API, please check your connection'));
                }
            });
    },
    ...
    _checkStatus(response) {
        ...
        return responseTextPromise.then((responseText) => {
            if (response.status === 403 && response.headers.map['x-ratelimit-remaining'] === '0') {
                ...
                errorText = this.intl.t('Unsplash API rate limit reached, please try again later.');
            }

            errorText = errorText || responseText || this.intl.t('Error {status}: Uh-oh! Trouble reaching the Unsplash API', {status: response.status});
